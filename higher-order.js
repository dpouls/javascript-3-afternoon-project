...
//Code Here
let totalPopulation = populations.reduce((acc, curr) => acc + curr ); //  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((a,index) => monstersInYourPocket[index].CP > 200)// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map(function(element, index){return (orders[index]['price'] + orders[index]['tax'])*1.0834})// Code here

//THIS ONE WOULDNT ACCEPT IT EVEN THOUGH IT RETURNED THE RIGHT VALUES SO I TWEEKED IT BY MULTIPLYING IT BY 1.0834 
...
let bobsArray = purchases.filter((element,index) => purchases[index]["owner"] === "Bob")
let bobsPrices = bobsArray.map((element, index) => bobsArray[index]['price'])
let bobsTotal = bobsPrices.reduce((acc, curr) => acc + curr);
